...
import { useUpdateCategory, useViewCategory } from "../../hooks/useCategories";
...
export default function UpdateCategory() {
  ...
  const onSubmit = (data: UpdateCategorySchema) => {
    if (!isDirty) return navigate("/categories");
    let payload = {
      name: "",
      description: "",
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-zinc-500 dark:text-zinc-400 text-sm flex items-center gap-1.5">
                  <Info size={14} /> Modify your category details and imagery.
                </p>
              </div>
            </div>
            <button
          ...
          {loading ? (
            <div className="space-y-7 animate-pulse">
              <div className="space-y-2">
                <div className="h-3 w-24 bg-zinc-200 dark:bg-zinc-800 rounded ml-1" />
                <div className="h-11 w-full bg-zinc-100 dark:bg-zinc-900 rounded-xl border border-zinc-200/60 dark:border-zinc-800/60" />
              </div>

              <div className="space-y-2">
                <div className="h-3 w-20 bg-zinc-200 dark:bg-zinc-800 rounded ml-1" />
                <div className="h-28 w-full bg-zinc-100 dark:bg-zinc-900 rounded-xl border border-zinc-200/60 dark:border-zinc-800/60" />
              </div>

              <hr className="border-zinc-100 dark:border-zinc-900" />

              <div className="space-y-2">
                <div className="h-3 w-28 bg-zinc-200 dark:bg-zinc-800 rounded ml-1" />
                <div className="h-40 w-full bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60" />
              </div>

              <div className="pt-2 flex gap-3">
                <div className="w-1/3 h-13 bg-zinc-100 dark:bg-zinc-900 rounded-xl border border-zinc-200/60 dark:border-zinc-800/60" />
                <div className="w-2/3 h-13 bg-zinc-200 dark:bg-zinc-800 rounded-xl" />
              </div>
            </div>
          ) : (
            <div className="space-y-7">
              ...
              <div className="space-y-1">
                ...
                {!imagePreview ? (
                  ...
                  <div className="relative rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden bg-zinc-100 dark:bg-zinc-900 h-80 group">
                    <img
                      ...
                      className="max-w-full w-full h-full object-cover object-center block"
                    />
                ...
                {errors.image?.message && (
                  <ErrorSpan message={errors.image.message} />
                )}
              </div>
              ...
              <div className="pt-2 flex gap-3">
                <button
                  type="button"
                  onClick={() => navigate("/categories")}
                  className="w-1/3 py-3.5 rounded-xl font-bold transition-all border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 active:scale-[0.99] text-[15px]"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="w-2/3 py-3.5 rounded-xl font-bold transition-all bg-black text-white dark:bg-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-100 active:scale-[0.99] text-[15px] shadow-lg shadow-black/5 disabled:bg-zinc-300 disabled:dark:bg-zinc-700 disabled:text-zinc-500 disabled:cursor-not-allowed"
                >
                  Save Changes
                </button>
              </div>
